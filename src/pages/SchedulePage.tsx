...
export default function SchedulePage({ branches, schedules, onSchedulesChange }: Props) {
  ...
  /* ── editable cell ── */
  function renderEditableCell(cell: ScheduleCell, ri: number, ci: number, isExport: boolean) {
  ...
  /* ── merged grid ── */
  function renderMergedGrid(rows: MergedRow[], isExport: boolean) {
  ...
  return (
    <div className="space-y-4" onClick={() => { if (colorPickerFor) setColorPickerFor(null); }}>
        ...
        <div className="flex gap-2 items-center relative">
          ...
          {showExportMenu && (
            <div className="absolute right-0 top-full mt-1 z-40 bg-white border border-border rounded-lg shadow-lg p-3 w-[220px]" onClick={(e) => e.stopPropagation()}>
              ...
              {([["showTitle", "Заголовок"], ["showGrid", "Расписание"], ["showFooter", "Контакты"]] as const).map(([k, l]) => (
                <label key={k} className="flex items-center gap-2 cursor-pointer mb-1.5">
                  <input type="checkbox" checked={exportOpts[k]} onChange={(e) => setExportOpts((o) => ({ ...o, [k]: e.target.checked }))} className="rounded" />
      ...
      <div ref={tableRef} className="bg-white rounded-2xl overflow-hidden"
        ...
        {(exporting ? exportOpts.showGrid : true) && (isMerged ? renderMergedGrid(mergedRows, exporting) : renderSingleGrid(hall?.timeSlots || [], exporting))}
        ...
        {(exporting ? exportOpts.showFooter : true) && (
          <div style={{ marginTop: exporting ? 8 : 6, borderTop: "1px solid #eee", paddingTop: 6, display: "flex", flexWrap: "wrap", gap: "4px 20px", alignItems: "center", justifyContent: "center" }}>
            {exporting ? (
              <>
                {schedule.address && <span style={{ fontSize: 10, color: "#777" }}>📍 {schedule.address}</span>}
                {schedule.phone && <span style={{ fontSize: 10, color: "#777" }}>📞 {schedule.phone}</span>}
                {schedule.nickname && <span style={{ fontSize: 10, color: "#777" }}>@ {schedule.nickname}</span>}
              </>
            ) : (
              <>
